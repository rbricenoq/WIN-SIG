...
$result = pg_Exec($conexion,'SELECT * 
	...
	wintig.rancheria.id_municipio = wintig.municipio.id_municipio and wintig.municipio.id_departamento = wintig.departamento.id_departamento
	order by
	id_fuente_hidrica');
...
if(pg_Num_Rows($result) > 0){
	...
	while($row = pg_fetch_assoc($result)){
		...
		<td><button onclick="detalles_fh('.$row['id_fuente_hidrica'].')" class="btn btn-warning">Editar</button></td>
		<td><button onclick="borrar_fh('.$row['id_fuente_hidrica'].')" class="btn btn-danger">Eliminar</button></td>
		</tr>';
		$number++;
	}
}
    	...
    	// records now found 
	$data .= '<tr><td colspan="73">No hay registros!</td></tr>';
}
$data .= '</table></div>';
echo $data;
?>